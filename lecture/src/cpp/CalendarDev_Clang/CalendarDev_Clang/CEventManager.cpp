...
CEventManager::read_info( string_container entry ) const
{
    ...
    // 年月日取り出し
    string_container date_str = CSV_Reader::Split( entry[ 0 ], '/' );
    sprintf( buf, "%s %s %s",
        date_str[ 0 ].c_str(), date_str[ 1 ].c_str(), date_str[ 2 ].c_str() );
    sscanf( buf, "%d %d %d",
        &( ret.date_.year ), &( ret.date_.month ), &( ret.date_.day ) );
    ret.date_.weekday = Cal_FormulaZeller( &( ret.date_ ) );
...
const CEventInfo
CEventManager::Search( EvnetInfoList list, DateInfo *search_date )
{
...
bool
CEventInfo::IsMatch( DateInfo *check ) const
{
...
{
} // EventListIndex_t::EventListIndex_t()
