...
出力イメージを示します。なお、レイアウトが崩れてしまいますので、MS明朝などの固定ピッチフォーマットでご覧下さい。プログラムでは、上の行から順番に出力する事になるので、概ね以下のような処理順序になります
...
を考えてみるとヒントが得られます。今回のケースであれば**どういった情報があれば、カレンダーを作成する事が出来る**のかを考えてみます。



少し考えてみた所、カレンダーは以下のルールで成り立っているようです
